...
import Tokenizer (isNumeralChar, isNotLetter, isNotValidSymbol)
...
eval exp env = case exp of
  ...
  LeqC e1 e2 ->
    case (eval e1 env, eval e2 env) of
      ...
      (_, _)                  -> error "ERRO eval EqC: um dos argumentos não é um número"
  LamC name body -> 
    if isReserved name then error "ERRO eval LetrecC: nome da expressão é reservado"
    else if isNotValidName name then error "ERRO eval LetrecC: nome da expressão possui formato inválido"
    else ClosV name body env
  AppC fun arg ->
    case closure of
  ...
  IfC cond b1 b2 ->
    case eval cond env of
      ...
      _ -> error "ERRO eval IfC: condição não é valor booleano"
...
isReserved name = name `elem` ["lambda", "call", "if", "let", "letrec", "cons", "head", "tail", "true", "false"] 

isNotValidName :: String -> Bool
isNotValidName name@(start : rest) 
  | isNotLetter start = True
  | otherwise = hasSymbols rest

hasSymbols :: String -> Bool
hasSymbols [] = False
hasSymbols s@(char : rest) =
  if isNotValidSymbol char then True
  else hasSymbols rest